...
if __name__ == '__main__':
    ...
    net.load_state_dict(torch.load(args.model_weights_path, map_location=torch.device('cpu')))
    net.eval()
    ...
    for idx, (volume, mask) in enumerate(dataloader):
        ...
        np.savetxt(os.path.join(patient_path, "dice_score.txt"), dice_score.numpy())
        output[output >= 0.5] = 1
        output[output < 0.5] = 0
        ...
        for slice_idx in range(output.shape[DEPTH]):
            ...
            filename_slice = 'slice_{}.png'.format(slice_idx)
            filename_mask = 'mask_{}.png'.format(slice_idx)
